...
# list *content-type*
Returns a list of content the current user can access on the server.

## Example

```tabcmd list projects```

```tabcmd list workbooks --details```

```tabcmd list datasources --owner --address --machine```

## Arguments

*content-type*
: The type of content to list. Must be one of: `projects`, `workbooks`, `datasources`, `flows`.

## Options

-n, \-\-name
: Include the item name in output. Shown alongside the item ID by default.

-o, \-\-owner
: Include the owner ID in output.

-d, \-\-details
: Print additional detail for each item. For workbooks, also lists up to 10 views contained in each workbook.

-a, \-\-address
: Include the content URL (web address) in output. Only applies to workbooks and datasources.

\-\-machine
: Emit output in a compact machine-readable format (comma-separated values without labels) suitable for scripting.
