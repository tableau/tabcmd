...
class AddUserCommand(UserCommand):
    ...
    def run_command(args):
        ...
        if args.require_all_valid:
            number_of_users_listed = UserCommand.validate_file_for_import(
                args.users, logger
            )
        logger.info(
            "Adding users listed in {0} to group '{1}'".format(
                args.users.name, args.name
            )
        )
        ...
        logger.info(
            "======== Number of users listed: {} =========".format(
                number_of_users_listed
            )
        )
        logger.info(
            "======== Number of users added: {} =========".format(number_of_users_added)
        )
